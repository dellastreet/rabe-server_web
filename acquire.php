...
$shellCommand = shell_exec("sudo ./rabe-server_acquire.sh ".$_SESSION['user']." ".$row_c['case_id']." ".$row_c['case_path']." ".$row_t['target_number']." ".$row_t['target_ip']." ".str_replace(':', '', strtolower($row_t['target_mac']))." ".$row_tc['targetConnect_blockDevice']." ".$row_tc['targetConnect_blockDeviceMount']);

$query = "UPDATE rabedb_targetConnect SET targetConnect_acquisition = '1' WHERE targetConnect_id = '".$row_tc['targetConnect_id']."';";
if ($mysqli->query($query) === TRUE) {
        echo "<link rel='stylesheet' type='text/css' href='rabe.css'>";
        echo "<body><p>Starting acquisition...<br />Record update succesful!</p></body>";
}
else {
