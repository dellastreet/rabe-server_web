...
$shellCommand = shell_exec("sudo ./rabe-server_stop_re-export.sh ".str_replace(':', '', strtolower($row_t['target_mac'])).":".$row_tc['targetConnect_blockDevice']." ".$row_tc['targetConnect_blockDeviceMount']." ".$row_c['case_path'].$row_t['target_number']."");

$query = "UPDATE rabedb_targetConnect SET targetConnect_reexportName = '' WHERE targetConnect_id = '".$row_tc['targetConnect_id']."';";
if ($mysqli->query($query) === TRUE) {
        echo "<link rel='stylesheet' type='text/css' href='rabe.css'>";
        echo "<body><p>Stopping re-export...<br />Record update succesful!</p></body>";
}
else {
