...
$query = "SELECT *, status_name FROM rabedb_request INNER JOIN rabedb_status ON request_status = status_id WHERE request_status != 1 AND request_organisation = 2;";
...
echo "<p><input type='button' onClick=javascript:location.href='all_requests_".$_SESSION['userOrganisation'].".php' id='allRequests' name='allRequests' value=' view all requests '>";

echo "<p><label>Open requests:</label></p>";
echo "<table>";
if (empty($rows_r)) {
        echo "<tr>no open requests</tr>";
}
else {
	echo "<tr><th>ID</th><th>Status</th><th>Number</th><th>Name</th><th>Investigator:</th><th></th><th></th></tr>";
	foreach($rows_r as $row_r) {
		...
		if ($row_c['case_locationType'] == '3') {
			echo "<td><input type='button' onClick=javascript:location.href='request_forward.php?request=".$row_r['request_id']."&case=".$row_c['case_id']."' id='forwardRequest' name='forwardRequest' value=' forward request '></td>";
		}
		else {
			echo "<td></td>";
		}
